...
//	@Override
	public void start(Stage primaryStage) throws IOException {
			...
			Map<String, Integer> frequencyOfWord = new HashMap<>();
		 	...
		 	String url      = "jdbc:mysql://localhost:3306/word_occurrences";   //database specific url.
	        String user     = "root";
	        String password = "abangaru";
	        ...
	        try(Connection connection = DriverManager.getConnection(url, user, password)) {
	            ...
	            try(Statement statement = connection.createStatement()){
	                ...
	                try(ResultSet result = statement.executeQuery(sql)){
	                    while(result.next()) {
	                        String poemWord = result.getString("word");

	                       // long   age  = result.getLong  ("age");
	                        System.out.println(poemWord);
	                    }
	                }
	            }
	        } 
			...
			String entry = word.getText();
			//using .get to find the frequency of the word user has entered
			int frequency = frequencyOfWord.get(entry);
